...
@Service
public class SensorAsyncTaskService{
    ...
    public Future<Float> executeAsyncTask(String device ) throws InterruptedException {
        ...
        sensorScore = random.nextFloat();
        //所用时间为random.nextInt(1000)
        Integer useTime = random.nextInt(1000);
        Thread.sleep(random.nextInt(useTime));

        redisTemplate.opsForHash().put(device, "sensor", String.valueOf(sensorScore));
...
//        System.out.println(redisTemplate.opsForHash().size("bbb"));
        ...
        System.out.println("线程" + Thread.currentThread().getName()  + " 设备号:"+device +" Get result score use time:"+useTime +" sensorScore："+ sensorScore+" save redis key：");
        return new AsyncResult<>(sensorScore);
    }
